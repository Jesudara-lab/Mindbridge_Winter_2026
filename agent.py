...
class QueryWriter:
    ...
    def generate_query(self, prompt: str) -> str:
        ...
        system_prompt = f"""You are a SQL expert working with a DuckDB bike store database.
    ...
    - Always include all SELECT columns in GROUP BY clause
    - Never use aliases in GROUP BY, use the actual column names
    - When you need data from multiple tables, always JOIN them properly
    - Always check which table a column belongs to before using it
    - For revenue calculations use: quantity * list_price * (1 - discount) from order_items
    - To get brand names you must JOIN: order_items -> products -> brands
    """
